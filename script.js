...
window.addEventListener("DOMContentLoaded", function () {
  for (let j = 0; j < this.localStorage.length; j++) {
    const rowLoad = RowElement(
      JSON.parse(localStorage.getItem(`Product${j + 1}`)).productName,
      JSON.parse(localStorage.getItem(`Product${j + 1}`)).price,
      JSON.parse(localStorage.getItem(`Product${j + 1}`)).qty
    );
    
    productArr.push({
      productName: JSON.parse(localStorage.getItem(`Product${j + 1}`))
        .productName,
...
btnAdd.addEventListener("click", () => {
  if (productName.value == "" && price.value == "" && qty.value == "") {
    ...
    const rowElement = RowElement(
      JSON.parse(localStorage.getItem(`Product${i + 1}`)).productName,
